...
Authors get a block library: a picker of 11 blocks, each one a sample document under [/tools/sidekick/blocks/](https://main--contitires--cloudadoption.aem.live/tools/sidekick/blocks/perfect-fit), wired into the DA editor through a `library` sheet in the DA site config, so blocks are inserted from a picker instead of being typed from memory. DA's Experience Workspace is switched on for the site. The [library app](tools/sidekick/library.html) and its [config](tools/sidekick/library.json) live in the repo; the sample content is authored in DA like everything else.

Each sample is one page: the block as an author would insert it, and a `library-metadata` block giving the picker its name, its description and its search terms. The newest two are the [banner](https://main--contitires--cloudadoption.aem.live/tools/sidekick/blocks/banner), the dark title band 13 standalone pages open with, and the [tire specs](https://main--contitires--cloudadoption.aem.live/tools/sidekick/blocks/tire-specs) band, the size selector and spec sheet on 44 product pages (#123). Pages author 20 blocks at the top of a section and the picker lists 10 of them. The ten with no entry are related articles (79 pages), video (63), tire listing (12), media gallery (8), share (8), crew (4), tire features, events, retailers and search. Two of those have a sample document already: the tire listing one is not in the index, and the video one names its fields in a `metadata` block rather than a `library-metadata` one, which the picker does not read. Size list is the eleventh entry and is on no page.
