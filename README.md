...
Live: [/Store-finder](https://continentaltire.com/Store-finder), store search over a map and a results list. Live serves the page empty and fills it from `/api/store-finder?lat=&lng=&dealerType=PLT&brands=continental`, a service on its own host, with `maps.googleapis.com` drawing the map. There is no public store database to scrape, so this site has no store search. A real version needs the dealer list (a DA sheet works for a static set), a geocoding service, and a lazily loaded map library; the block then queries the data client-side.

`/Store-finder` redirects to [online retailers](https://main--contitires--cloudadoption.aem.live/online-retailers) here, which is where the site's Find Stores links land. Its Store Near You tab held a mock: a search box the [store-locator block](blocks/store-locator/store-locator.js) disables, and one result reading 3.4 miles to Continental's own head office in Fort Mill. A control nobody can type into, and a distance measured to nothing, read as a broken search rather than as a demo. The tab reads `Store search is not part of this site` now, says what a search would need, and points at the Online Retailers tab beside it. The mock is still on the tires hub and the 46 product pages, where the result reads as a nearby dealer.
