...
The picker reads `library-metadata` per section. A sample keeps its name and its search terms only where the metadata shares a section with the block. Six were in other shapes until 2026-07-29. All fourteen share one section now, so the picker names every one. It searches names and search terms, not descriptions. The block ships [its own stylesheet](blocks/library-metadata/library-metadata.css) and a script that does nothing. The loader fetches both for every block it decorates. Without them, each of the fourteen sample pages logged two 404s and a failed import (#285).
