...
Each sample is one page: the block as an author would insert it, and a `library-metadata` block giving the picker its name, its description and its search terms. The newest two are the [banner](https://main--contitires--cloudadoption.aem.live/tools/sidekick/blocks/banner), the dark title band 13 standalone pages open with, and the [tire specs](https://main--contitires--cloudadoption.aem.live/tools/sidekick/blocks/tire-specs) band, the size selector and spec sheet on 44 product pages (#123). Seven blocks that pages use have no entry: related articles, video, share, media gallery, events, retailers and search. A video sample document exists, but it names its fields in a `metadata` block rather than a `library-metadata` one, and the picker does not read that.
