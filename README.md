...
`/Store-finder` redirects to [online retailers](https://main--contitires--cloudadoption.aem.live/online-retailers) here, which is where the site's Find Stores links land. The same mock was on 49 pages: a search box the [store-locator block](blocks/store-locator/store-locator.js) disables, and one frozen result. The result is live's Fort Mill default, transcribed. On /online-retailers it was Continental's own head office. On the other 48, the homepage, the tires hub and 46 product pages, it was Mavis Tires & Brakes Rock Hill at 3.29 miles, a real independent dealer, with that dealer's street address and phone number. Live recomputes that on a reader's location and this site cannot, so 3.29 miles was fixed text on a page any reader can reach. All 49 read `Store search is not part of this site` now, say what a search would need, and link to the online retailers page. The block is on no page.
