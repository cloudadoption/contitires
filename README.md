...
The picker reads `library-metadata` per section. A sample keeps its name and its search terms only where the metadata shares a section with the block. Six were in other shapes until 2026-07-29. All fourteen share one section now, so the picker names every one. It searches names and search terms, not descriptions.
