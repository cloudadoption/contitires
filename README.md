...
Authors get a block library: a picker of 14 blocks, each one a sample document under [/tools/sidekick/blocks/](https://main--contitires--cloudadoption.aem.live/tools/sidekick/blocks/perfect-fit), wired into the DA editor through a `library` sheet in the DA site config, so blocks are inserted from a picker instead of being typed from memory. DA's Experience Workspace is switched on for the site. The [library app](tools/sidekick/library.html) and its [index](tools/sidekick/library.json) live in the repo. The samples are authored in DA, and so is the index the DA site config names, `/library/blocks.json`. That one is read from `content.da.live` with the author's own token rather than published like a page, which is why it answers 404 on both hosts. Both indexes list the same 14 in the same order.

Each sample is one page: the block as an author would insert it, and a `library-metadata` block giving the picker its name, its description and its search terms. The newest three are the three widest gaps (#123): [related articles](https://main--contitires--cloudadoption.aem.live/tools/sidekick/blocks/related-articles), the further reading list on 79 pages, [video](https://main--contitires--cloudadoption.aem.live/tools/sidekick/blocks/video), the click to load player on 63, and [tire listing](https://main--contitires--cloudadoption.aem.live/tools/sidekick/blocks/tire-listing), the filterable catalog on 12. Pages author 20 blocks at the top of a section and the picker lists 13 of them, with size list a fourteenth that is on no page. Seven are still missing: media gallery (8 pages), share (8), crew (4), tire features, events, retailers and search.

Which section the metadata is in decides what the picker gets from it. A sample whose `library-metadata` shares a section with the block is named by it and searchable by its terms. Split the two into separate sections and the search terms still arrive, but the sample is labelled by its path: banner, cards, columns and tire specs. Put the metadata outside a section and nothing arrives, which is carousel and hero showing as `Unnamed Item`. The picker searches names and search terms, not descriptions.
...
Live: [/Store-finder](https://continentaltire.com/Store-finder), store search over a map and a results list. Live serves the page empty and fills it from `/api/store-finder?lat=&lng=&dealerType=PLT&brands=continental`, a service on its own host, with `maps.googleapis.com` drawing the map. Live puts the same service behind a `Stores near` band on its homepage and on every product page. That band has a working search box above its results. Neither ships a store in its HTML. Given no location from the reader, live falls back to `35.0074,-80.9451`, its own head office in Fort Mill, and queries the service for that. So a first-time visitor anywhere in the world is shown the store nearest Fort Mill, and `hide-default` keeps Fort Mill out of the search box, so the page does not say what the distance is from. There is no public store database to scrape, so this site has no store search. A real version needs the dealer list (a DA sheet works for a static set), a geocoding service, and a lazily loaded map library; the block then queries the data client-side.

`/Store-finder` redirects to [online retailers](https://main--contitires--cloudadoption.aem.live/online-retailers) here, which is where the site's Find Stores links land. The same mock was on 49 pages: a search box the [store-locator block](blocks/store-locator/store-locator.js) disables, and one frozen result. The result is live's Fort Mill default, transcribed. On /online-retailers it was Continental's own head office. On the other 48, the homepage, the tires hub and 46 product pages, it was Mavis Tires & Brakes Rock Hill at 3.29 miles, a real independent dealer, with that dealer's street address and phone number. Live recomputes that on a reader's location and this site cannot, so 3.29 miles was fixed text on a page any reader can reach. All 49 read `Store search is not part of this site` now, say what a search would need, and link to the online retailers page. The block is on no page, and it is not in the block library. Its one authored row is the store result, so a library sample would put a frozen dealer one click away.
