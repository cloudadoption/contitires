...
**A production build has to replace this.** Stag Sans is licensed, and a hotlink is a dependency on a host this project does not own. Tighten that CORS header or move the theme directory and the site falls through to Arial. License the four faces, commit them under `fonts/`, change four URLs, and it is done. `fonts/` today contains the boilerplate's four Roboto woff2 files, which no stylesheet references.

The metric-matched fallback is declared in `styles.css` rather than in `fonts.css`, and the split is what makes it work. `head.html` links `styles.css` render-blocking, while `loadFonts()` fetches `fonts.css` over the network. A fallback prevents a shift only if it exists at the first paint. Declared in `fonts.css` it was an unknown family until that fetch landed. The first paint fell through to plain Arial and the page reflowed anyway. `/accessibility-statement` measured CLS 0.2761, with its paragraphs moving 29px at 81ms.
...
Whether `fonts.css` is requested in the eager phase depends on viewport width. `loadsFontsEagerly()` in [`scripts/scripts.js`](../scripts/scripts.js) returns `window.innerWidth >= 769`, so a wide viewport paints in Stag while a narrow one paints in the fallback and swaps. Width is a proxy for a fast connection, and the number is 769 for the reason the breakpoint section gives. A `sessionStorage` flag used to be a second term in that test. It made one URL at one width paint two ways, and the tab's history picked which. A fresh tab took the fonts lazily at FCP + 1155ms. A tab that had already shown a page took them eagerly at FCP - 2ms. `loadFonts()` still writes the flag, and no code reads it.
