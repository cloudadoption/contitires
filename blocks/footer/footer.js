...
const SOCIAL_ICONS = {
  ...
  'twitter.com': 'M14.234 10.162 22.977 0h-2.072l-7.591 8.824L7.251 0H.258l9.168 13.343L.258 24H2.33l8.016-9.318L16.749 24h6.993zm-2.837 3.299-.929-1.329L3.076 1.56h3.182l5.965 8.532.929 1.329 7.754 11.09h-3.182z',
  'x.com': 'M14.234 10.162 22.977 0h-2.072l-7.591 8.824L7.251 0H.258l9.168 13.343L.258 24H2.33l8.016-9.318L16.749 24h6.993zm-2.837 3.299-.929-1.329L3.076 1.56h3.182l5.965 8.532.929 1.329 7.754 11.09h-3.182z',
...
/**
 * Finds the social icon path data for a link, matched by hostname.
 * @param {HTMLAnchorElement} link the authored link
 * @returns {string|null} SVG path data, or null when the link isn't a known social profile
 */
function getSocialIcon(link) {
  try {
    const host = new URL(link.href).hostname.replace(/^www\./, '');
    return SOCIAL_ICONS[host] || null;
  } catch {
    return null;
  }
}
 ...
 * @param {HTMLElement} group a footer links group (heading + list)
 */
function decorateSocialGroup(group) {
  const list = group.querySelector('ul');
  if (!list) return;
  const items = [...list.querySelectorAll('a')];
  if (!items.length) return;
  const icons = items.map(getSocialIcon);
